...
func (_m *StateBasedValidator) Validate(cc string, blockNum uint64, txNum uint64, rwset []byte, prp []byte, ep []byte, chdr []byte, endorsements []*peer.Endorsement, vrfs []*protospeer.VrfEndorsement) errors.TxValidationError {
	ret := _m.Called(cc, blockNum, txNum, rwset, prp, ep, chdr, endorsements, vrfs)

	var r0 errors.TxValidationError
	if rf, ok := ret.Get(0).(func(string, uint64, uint64, []byte, []byte, []byte, []byte, []*peer.Endorsement, []*protospeer.VrfEndorsement) errors.TxValidationError); ok {
